RedSwitchBlockClass = EntityClass.extend({
    zIndex : 50,
    myPhysBody: null,

    init: function (inputx, inputy, settings) {
        this.currSpriteName = 'switch_red_off.png';
        this.pos.x = 200;
        this.pos.y = 300;
        this.hpos.x = 235;
        this.hpos.y = 323;
    ...
    onTouch: function(otherBody, point, impulse){
        ...
        if(data.id == 'lock_red' && this.myPhysBody.GetUserData().activated){
            otherBody.SetType(Body.b2_dynamicBody);
            otherBody.SetLinearVelocity(new Vec2(0,-100));
            data.activated = true;
            this.myPhysBody.SetLinearVelocity(new Vec2(0,-100));
        }
...
gGameEngine.factory['switch_red_off'] = RedSwitchBlockClass;